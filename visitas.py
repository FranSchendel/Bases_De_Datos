...
def ingresa_visita(persona):
    ...
    if resu.fetchone():
        ...
        conn.commit()
    conn.close()
    persona.entrada = datetime.datetime.now().replace(microsecond=0).isoformat()
...
def egresa_visita(dni):
    ...
    q = f"""INSERT INTO ingresos_egresos (fechahora_out)
...
def lista_visitantes_en_institucion ():
    ...
    q = f"""SELECT dni FROM ingresos_egresos WHERE fechahora_out = NULL;"""
    for i in q:
        a = f"""SELECT * FROM personas WHERE dni = {i}"""
        print(a)
...
def busca_vistantes(fecha_desde = "", fecha_hasta = "", destino = "", dni = ""):
    ...
    conn = sqlite3.connect('recepcion.db')
    q = f"""SELECT nombre, apellido 
            FROM personas
            WHERE fechahora_in = '{fecha_desde}' or fechahora_out = '{fecha_hasta}' or destino = '{destino}' or dni = '{dni}' """

    criterios = conn.execute(q)
